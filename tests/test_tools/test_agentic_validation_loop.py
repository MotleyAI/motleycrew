...
class TestAgenticValidationLoop:
    ...
    def test_init_with_prompt_template(self, mock_llm):
        ...
        tool = AgenticValidationLoop(
            name="test_tool", description="A test tool", prompt=prompt_template, llm=mock_llm
        )
    ...
    def test_run_method(self, mock_structured_tool, mock_agent_class, mock_llm):
        ...
        assert call_kwargs["prompt"] == "Test prompt with test_value"
    ...
    def test_post_processing(self, mock_llm):
        ...
        with patch(
            "motleycrew.tools.agentic_validation_loop.ReActToolCallingMotleyAgent"
        ) as mock_agent_class:
            mock_agent_instance = MagicMock()
            ...
            with patch(
                "motleycrew.tools.agentic_validation_loop.StructuredPassthroughTool"
